// ============================================================================
// 06 - Multithreading: 增强版线程池 (Enhanced Thread Pool)
// ============================================================================
// 在基础线程池上增加 future 返回、任务优先级、优雅停止等特性。
// ============================================================================
...
public:
    ...
    ThreadPoolStats getStats() const {
        ...
        return {
            ...
            finishedTasks.load(),
            taskQueue.size(),
            static_cast<double>(runTime.count())  // 添加显式转换
        };
    }
...
void basicTaskExample() {
    ...
    for (int i = 0; i < 10; ++i) {
        results.push_back(pool.enqueue([i] {
            this_thread::sleep_for(chrono::milliseconds(dis(gen) * 20));
            lock_guard<mutex> lock(coutMutex);  // 保护输出
            cout << "任务 " << i << " 执行完成" << endl;
            return i * i;
        }));
    }
